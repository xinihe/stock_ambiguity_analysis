...
%% Abstract
\begin{abstract}
\textcolor{red}{We develop a new cross-entropy-based ambiguity index ($\mathcal{A}^{CEA}_t$) using high-frequency intraday return distributions. Unlike traditional risk measures, $\mathcal{A}^{CEA}_t$ captures Knightian uncertainty by reflecting the absence of a reliable probability model rather than merely measuring outcome variance. Using minute-level CSI 300 data between 2018 and 2024, we find that elevated ambiguity predicts higher next-day returns, with effects concentrated in high-volatility regimes. Mediation analysis reveals a liquidity provision channel whereby ambiguity induces market makers to widen spreads, generating a liquidity premium that translates into higher subsequent returns. Portfolios conditioned on $\mathcal{A}^{CEA}_t$ outperform volatility-based strategies. Our findings suggest that ambiguity acts as a separate and economically significant driver of asset prices in emerging markets, independent of traditional volatility.}
\end{abstract}
...
\textcolor{red}{Financial markets are complex systems operating under conditions of pervasive uncertainty. This paper investigates how ambiguity, which is uncertainty about the probability distributions of returns themselves, affects equity prices in China's A-share market. The Chinese A-share market is particularly suitable for studying this phenomenon for three reasons. First, high retail participation, comprising over 80\% of trading volume, introduces substantial investor heterogeneity and behavioral biases \citep{guo2020retail}, which generates divergent beliefs and creates fertile ground for ambiguity effects. Second, frequent policy interventions and geopolitical exposure, such as US-China trade tensions, generate sharp distributional shifts that are highly visible in intraday data. Third, the absence of T+0 trading and the prominent role of market-making desks in this market make liquidity provision responses to ambiguity especially interpretable.}

\textcolor{red}{Despite extensive research on volatility and risk premiums, the economics literature has not established how Knightian ambiguity, as distinct from outcome variance, transmits to equity returns at high frequency. Traditional risk measures evaluate the variance of outcomes based on known probabilities, such as standard Value-at-Risk, failing to capture deeper forms of uncertainty where the probability distributions themselves are unknown or misspecified. Most existing studies treat uncertainty as a monolithic construct or rely on indirect proxies, such as Expected Utility Under Uncertainty or Choquet Expected Utility, that are primarily theoretical, rely on non-linear optimization, or cannot separate model uncertainty from outcome uncertainty. These established paradigms struggle to capture the full-shape changes of return distributions, leaving a significant gap in dynamic portfolio and risk management \citep{Bollerslev1988}.}


\textcolor{red}{To address this gap, this paper is the first to (i) construct a daily, cross-sectionally applicable ambiguity index, denoted as $\mathcal{A}^{CEA}_t$, from intraday return distributions; (ii) demonstrate its separate pricing effect from realized volatility and higher moments; and (iii) identify the liquidity provision channel as the dominant mechanism in emerging markets. The index is designed for real-time monitoring, as it can be calculated and implemented in a timely manner. \footnote{\textcolor{red}{Unless otherwise specified, `short-term' in this paper refers to the next trading day (day $t+1$). The ambiguity index $\mathcal{A}^{CEA}_t$ is constructed using intraday data from trading day $t$ and is available before the market opens on day $t+1$. All return predictions and portfolio signals are implemented at the close of day $t$ and realized at the close of day $t+1$.}} Furthermore, this study is particularly relevant to the special issue on technological innovation and cross-border activities for two reasons. First, as geopolitical risk is a core driver of cross-border investment uncertainty, it creates ambiguity about probability distributions that standard GARCH or volatility models cannot capture. Our $\mathcal{A}^{CEA}_t$ index directly measures this distributional uncertainty. Second, firms engaged in technology-intensive sectors and cross-border operations face return distributions that are inherently harder to model, making ambiguity monitoring essential for portfolio risk management in these segments. Our methodology applies directly to these firms' high-frequency data, offering a new tool for researchers and practitioners studying the intersection of innovation, internationalization, and firm-level uncertainty.}


\textcolor{red}{Theoretically, we extend the Hansen-Sargent multiplier-preference framework \citep{hansen2001} to a data-adaptive, information-theoretic implementation that is directly estimable from market prices. The multiplier-preference framework models decision-making under uncertainty by evaluating options against a worst-case prior within a set of plausible distributions \citep{maccheroni2006, epstein2008}. We operationalize this using cross-entropy, which precisely quantifies the information lost when a nominal probability distribution is used to approximate the true, unknown distribution \citep{klibanoff2005}. Because minimizing cross-entropy is equivalent to minimizing the Kullback-Leibler (KL) divergence between empirical and benchmark distributions \citep{mackay2003information}, our approach serves as a direct penalty for deviations from nominal priors, translating theoretical ambiguity aversion into a quantifiable, data-driven metric.}

\textcolor{red}{Empirically, we exploit the Chinese A-share market's high-frequency microstructure to identify a robust, regime-dependent ambiguity premium that existing volatility-based models miss entirely. This paper investigates two primary mechanisms through which ambiguity influences asset prices. First, in high-ambiguity regimes, market makers widen bid-ask spreads and reduce order book depth as compensation for model uncertainty, creating a liquidity premium. Second, sophisticated investors treat elevated ambiguity as a signal of distributional uncertainty, demanding higher expected returns as compensation for holding assets whose return distributions are difficult to model. We validate these channels using conditional mediation analysis and conduct a comprehensive set of causal econometric evaluations. These evaluations include Granger causality, event studies around exogenous shocks, and heterogeneity analysis across market regimes, collectively establishing that $\mathcal{A}^{CEA}_t$ has a distinct, independent effect on future returns.}

\textcolor{red}{We operationalize ambiguity through the cross-entropy divergence between intraday and historical return distributions. Our key finding is that this ambiguity acts as a separate and significant predictor of next-day returns, with its effects primarily concentrated in high-volatility and high-liquidity regimes. Both the liquidity and expected-return channels predict a positive relationship between ambiguity and next-period returns, a result strongly supported by our empirical findings. The positive ambiguity premium indicates that investors are indeed compensated for bearing distributional uncertainty, and portfolio strategies incorporating this index consistently outperform traditional risk-based approaches.}

\textcolor{red}{The remainder of this paper is structured as follows. Section \ref{sec:literaturerev} provides a comprehensive review of the theoretical foundations of ambiguity and existing measures. Section \ref{sec:researchdes} details the methodology for constructing $\mathcal{A}^{CEA}_t$. Section \ref{sec:empirical} describes the data and variables. Section \ref{sec:emiricalmet} presents the empirical methodology and results, including the econometric analyses. Section \ref{sec:portfoliobac} reports the portfolio backtest and performance evaluation. Section \ref{sec:conclusion} concludes with a summary of findings and outlines directions for future research.}
...
Since the pioneering work of \cite{neumann1947}, Expected Utility Theory (EUT) has been widely applied to model investor attitudes toward risk. In Markowitz's seminal mean–variance framework \citep{markowitz1952}, risk entered portfolio optimization via variance, reshaping modern finance and laying the groundwork for dynamic strategies by \cite{merton1971} and others. \cite{savage1954} further formalized the axiomatic foundations of EUT, yet experimental anomalies such as the prominent Allais paradox \citep{allais1953} exposed limitations in capturing real decision behavior.

\cite{knight1921} distinguished risk (known probabilities) from uncertainty (unknown probabilities), a differentiation powerfully illustrated by Ellsberg's urn experiments \citep{ellsberg1961}. In this setup, decision makers choose between Urn I, containing an unknown composition of red and black balls, and Urn II, with a known 50:50 ratio. When drawing for a potential gain (red ball), participants showed a clear preference for Urn II; conversely, when tasked with avoiding a loss (black ball), they sometimes favored Urn I despite the ambiguity. These context‑dependent choices violate Savage's sure‑thing principle and demonstrate that ambiguity, defined as uncertainty about probability distributions, affects preferences beyond measurable risk.
...
\paragraph{Subjective Dispersion Measures.} Early ambiguity quantification focused on the variability of investors' subjective probability estimates. \cite{izhakian2020} proposed the EUUP framework, which defines an ambiguity metric as:
\begin{equation}
...
\paragraph{Choquet and Multiple Priors} The Choquet Expected Utility (CEU) model proposed by \cite{schmeidler1989} utilizes non‑additive capacities and the Choquet integral to incorporate ambiguity into decision weights. Under this model, the utility of an act $f$ is:
\begin{equation}
U_{\mathrm{CEU}}(f)=\int_0^\infty v\bigl(P(f\ge x)\bigr)\,dx + \int_{-\infty}^0 \bigl[v\bigl(P(f\ge x)\bigr)-1\bigr]\,dx,
\end{equation}
where $v(\cdot)$ is a convex capacity reflecting ambiguity attitudes. Alternatively, the Maxmin Expected Utility (MEU) model developed by \cite{gilboa1989} assumes a decision maker evaluates acts against the worst‑case prior $p^*\in\mathcal{P}$:
\begin{equation}
...
\cite{hansen2001} incorporate a divergence term into the value functional within their multiplier preferences model:

\begin{equation}
V(f)=\min_{p\in\mathcal{P}}\left\{\mathbb{E}_p[U(f)] + \rho\,D_{\mathrm{KL}}(p\,\|\,q)\right\},
\end{equation}
where $\rho>0$ calibrates aversion to ambiguity. Variational preferences generalize this via a penalty function $c(p)$ \citep{maccheroni2006}:
\begin{equation}
...
Since the Ellsberg experiment revealed the importance of ambiguity, subsequent research has examined its role in financial markets. \citet{anderson2009} measure ambiguity using forecaster disagreement and document a strong link between uncertainty and excess returns, with an uncertainty premium that improves the explanatory power of standard factor models. \citet{rieger2012} analyze international survey evidence and show that ambiguity aversion helps account for the equity premium puzzle, with higher aversion associated with higher premiums across countries. \citep{ariel2014} apply multiple-prior expected utility and find that ambiguity is priced independently of traditional risk factors, outperforming Bayesian learning, time-varying ICAPM, and Fama–French–Carhart models in explaining the cross-section of returns. Using changes in the VIX index, \citep{williams2015} quantify time variation in ambiguity. \cite{xu2016} employ proxies such as opinion divergence, abnormal turnover, cash-flow volatility, and disclosure quality to capture information ambiguity and show that ambiguity aversion explains momentum in China's stock market. Across 21 international markets, \cite{kim2021} report that accounting for ambiguity yields a positive association between risk and the equity premium; expected return probabilities shape investors' ambiguity attitudes, with stronger effects in emerging markets and lower-transparency environments. \cite{hu2022} further show that favorable expectations generate a positive model premium (ambiguity aversion) alongside a negative risk premium (risk preference), consistent with momentum behaviors such as chasing winners and selling losers.

\textcolor{red}{Our paper contributes to this growing literature in two distinct ways. First, unlike prior studies that use survey-based or volatility-based proxies for uncertainty \citep{xu2016,kim2021}, our $\mathcal{A}^{CEA}_t$ measure operationalizes Knightian uncertainty directly through distributional divergence, capturing model-form ambiguity rather than outcome variance. Second, whereas existing economics and finance studies have documented ambiguity effects across multiple markets \citep{doi:10.1093/oep/gpae004,doi:10.1007/s10479-021-04314-7}, we provide the first high-frequency, intraday implementation that adapts the benchmark distribution dynamically to changing market regimes, enabling daily construction across the entire cross-section. This methodological novelty opens avenues for real-time risk management applications that were previously infeasible.}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X p{0.35\textwidth}}
    ...
    \midrule
    CEU \citep{schmeidler1989} & Non-additive subjective beliefs (capacities) & Applies Choquet integral to model preference under ambiguity without additive probabilities \\
    MEU \citep{gilboa1989} & Multiple prior distributions & Uses worst-case utility evaluation to capture ambiguity-averse behavior \\
    KL Divergence \citep{hansen2001} & Multiplier / Variational Preferences & Introduces relative entropy as penalty for deviation from benchmark model \\
    EUUP \citep{izhakian2020} & Dispersion of subjective probabilities & Links density variance to ambiguity by combining expectation and volatility of return densities \\
...
To quantify ambiguity, we apply the KL divergence, a well-established measure in information theory that quantifies the difference between two probability distributions. The KL divergence is defined as:
...
The final ambiguity measure reflects the divergence between the empirical data, specifically the daily return distribution \(q_{\text{day}}\), and the subjective beliefs encoded in the benchmark \(p_i\). The higher the divergence, the greater the ambiguity the decision-maker faces in predicting future returns.
...
\begin{figure}[!htbp]
    \centering
    \textcolor{red}{\includegraphics[width=1\linewidth]{figs/uncertainty_visualization.png}}
...
\textcolor{red}{Hypothesis: Ambiguity aversion predicts that investors demand compensation for holding assets whose return distribution is uncertain. This implies a positive $\beta_{\mathcal{A}}$ in forward return regressions, beyond the effect of realized volatility, meaning that higher ambiguity commands a higher expected return as compensation. Consistent with this prediction, Table \ref{tab:panelols_model2} shows that a one-standard-deviation increase in $\mathcal{A}^{CEA}_t$ is associated with a 7 basis-point increase in next-day return (t=7.88), after controlling for RV, skewness, kurtosis, and turnover. This result is economically meaningful: it suggests that ambiguity captured by distributional uncertainty, which is not captured by conventional risk measures, is a separate pricing factor in China's A-share market.}
...
\begin{equation}
\begin{aligned}[b]
r_{t+1} =& \alpha + \beta_1 \cdot \mathcal{A}^{CEA}_t + \delta_{\mathrm{bull}} \cdot \mathrm{bull\_market}  \\
& + \delta_{\mathrm{bear}} \cdot \mathrm{bear\_market} + \gamma_1 \cdot \mathrm{Controls}_t + \varepsilon_{t+1}
\end{aligned}
\end{equation}

Here, $\mathrm{bull\_market}$ and $\mathrm{bear\_market}$ are dummy variables indicating bull and bear market phases, respectively. The coefficients $\delta_{\text{bull}}$ and $\delta_{\text{bear}}$ capture the differential impact of market regimes on the returns beyond the effect of ambiguity and control variables.
...
\begin{figure}[ht]
\centering
\textcolor{red}{\includegraphics[width=0.9\linewidth]{figs/figure4_quantile_coefficients.pdf}}
...
\begin{figure}[!htbp]
    ...
    \begin{subfigure}[b]{0.49\linewidth}
        \centering
        \textcolor{red}{\includegraphics[width=\linewidth]{figs/AMBE deciles.png}}
    ...
    \begin{subfigure}[b]{0.49\linewidth}
        \centering
        \textcolor{red}{\includegraphics[width=\linewidth]{figs/AMBMP Deciles.png}}
...
Figure \ref{fig:ambe-deciles} presents the cumulative net-value performance of decile portfolios and the long-short strategy constructed using AMBE . From the figure, we observe that the long-short portfolio (Q9--Q0)
starts at 1.0 in 2018 and rises to approximately 4.2 by 2024, exhibiting a pronounced upward trend. High decile portfolios (e.g., Q9) show steadily increasing cumulative net values, whereas low decile portfolios (e.g., Q0) remain relatively flat or even decline slightly. This indicates that the AMBE-based factor possesses substantial discriminative power, effectively capturing the differing risk and return characteristics of assets. Figure \ref{fig:ambmp-deciles} depicts the cumulative net-value performance of decile portfolios and the long-short strategy constructed using the proposed \(\mathcal{A}^{CEA}_t\) method. It
...
Figure~\ref{fig:strategy_vs_index_a} presents the performance of the long-short strategy constructed using the \(\mathcal{A}^{CEA}_t\) method versus the HS300 index. As shown, the \(\mathcal{A}^{CEA}_t\) strategy exhibits a pronounced and sustained upward trend, increasing from an initial value of 1 in 2018 to approximately 11.5 in 2024. In contrast, the HS300 index grows from 1 to only about 2.1 during the same period, reflecting a substantially smaller gain. The \(\mathcal{A}^{CEA}_t\) strategy consistently outperforms the benchmark index throughout the sample period, with the performance gap widening notably after 2021. This indicates the \(\mathcal{A}^{CEA}_t\) method's strong advantage in delivering superior risk-adjusted returns.
\begin{figure}[!htbp]
  \centering
  \begin{subfigure}[b]{0.49\linewidth}
    \centering
    \textcolor{red}{\includegraphics[width=\linewidth]{figs/AMBE long-short.png}}
  ...
  \begin{subfigure}[b]{0.49\linewidth}
    \centering
    \textcolor{red}{\includegraphics[width=\linewidth]{figs/AMBMP Long-short.png}}
...
The CEA index is grounded in Hansen and Sargent's multiplier-preference model \citep{hansen2001}, where a decision-maker evaluates robustness by considering worst-case distributions within a KL-divergence ball around a reference model: $V(f) = \min_{p \in \mathcal{P}} \{\mathbb{E}_p[U(f)] + \rho D_{KL}(p\|q)\}$, where $\rho > 0$ represents ambiguity aversion. The CEA index $\mathcal{A}^{CEA}_t$ operationalizes this framework by measuring the KL divergence between empirical intraday return distributions and historically plausible benchmarks.
...
Traditional ambiguity measures like EUUP \citep{izhakian2020} that quantify ambiguity as $\mho^2 = \mathbb{E}[\varphi(r)] \times \mathrm{Var}[\varphi(r)]$ suffer from several limitations. They capture only the second moment of probability densities, have low sensitivity to tail events due to quadratic penalty, and cannot handle multimodal distributions effectively. In contrast, the cross-entropy approach provides full-distribution comparison with superior interpretability and dynamic adaptation capabilities.
...
\begin{figure}[!htbp]
    ...
    \begin{subfigure}[t]{0.49\linewidth}
        \centering
        \textcolor{red}{\includegraphics[width=\linewidth]{figs/figure1_kl_divergence.pdf}}
    ...
    \begin{subfigure}[t]{0.49\linewidth}
        \centering
        \textcolor{red}{\includegraphics[width=\linewidth]{figs/figure4_risk_vs_ambiguity.pdf}}
...
\textcolor{red}{Sensitivity to intraday discretization. Constructing $\mathcal{A}^{CEA}_t$ from intraday return distributions requires discretizing the trading day into bins. As is standard in high-frequency measurement, this choice involves a trade-off: very fine sampling can amplify market microstructure noise and sparse-trading effects, while very coarse sampling can smooth away genuine intraday distributional shifts. To assess robustness, we recompute $\mathcal{A}^{CEA}_t$ using substantially finer and coarser schemes, including 5-, 15-, 30-, 60-, and 120-minute returns, as well as an extreme daily (close-to-close) discretization. Across these alternatives, the time-series and cross-sectional rankings of $\mathcal{A}^{CEA}_t$ remain highly similar to the baseline measure, and the main empirical conclusions are unchanged: $\mathcal{A}^{CEA}_t$ continues to predict next-day returns with a positive and statistically significant coefficient after controlling for realized volatility and higher moments, and the liquidity-channel evidence remains qualitatively the same.}
