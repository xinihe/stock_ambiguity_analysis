...
\textcolor{red}{We develop a novel cross-entropy-based ambiguity index ($\mathcal{A}^{CEA}_t$) using high-frequency intraday return distributions to dynamically quantify Knightian uncertainty. Grounded in the multiplier-preference framework, $\mathcal{A}^{CEA}_t$ captures the information divergence between empirical distributions and benchmark models, isolating model misspecification from traditional outcome variance. Using minute-level market data (2018--2024), we document a robust, positive relationship between elevated ambiguity and subsequent short-term returns. Conditional mediation analysis identifies a liquidity provision channel, wherein market makers demand a premium to absorb distributional uncertainty. Portfolios conditioned on $\mathcal{A}^{CEA}_t$ systematically outperform conventional volatility-based strategies. Ultimately, our methodology provides a highly tractable tool for distributionally robust portfolio optimization and real-time risk management, establishing that ambiguity is a fundamental, independently priced driver of asset returns in complex financial environments.}
\end{abstract}
...
Figure~\ref{fig:quantile_coefficients} displays the quintile-specific $\beta_q$ coefficients. The coefficient rises from 0.001 in Q1 (insignificant, $p = 0.276$) to 0.004 in Q3 ($p < 0.01$) and 0.007 in Q5 ($p < 0.01$), indicating that when ambiguity is high, $\mathcal{A}^{CEA}_t$ becomes a stronger predictor of positive returns. This pattern is absent in the lowest ambiguity groups (Q1 and Q2), where coefficients are small and not significant.
