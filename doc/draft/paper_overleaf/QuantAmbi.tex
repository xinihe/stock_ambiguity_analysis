...
\[
\mathrm{AMB}_{\mathrm{MP}} \;=\; D\bigl(q_{\mathrm{day}} \parallel p_i\bigr).
\]
...
To test for the potential predictive relationship between the ambiguity measure $ \text{AMBMP} $ and future returns, we conducted a Granger causality test. The results for lag orders 1 to 5 are summarized in Table~\ref{tab:granger} below.
...
To further investigate the predictive power and stability of the ambiguity measure (AMBMP) across varying market environments, we conduct a series of subgroup analyses. This approach allows us to examine whether the relationship between ambiguity and future returns is influenced by distinct market states, levels of volatility, or price conditions. Understanding these heterogeneous effects is crucial for assessing the robustness and practical applicability of AMBMP in real-world portfolio management and risk assessment.
...
\begin{equation}
r_{t+1} = \alpha + \beta_1 \cdot \mathrm{AMBMP}_t + \gamma_1 \cdot \mathrm{Controls}_t + \delta_{\text{bull}} \cdot \text{bull\_market} + \delta_{\text{bear}} \cdot \text{bear\_market} + \varepsilon_{t+1}
\end{equation}
...
\begin{table}[h!]
\centering
\caption{AMBMP Regression Results with Market Regime Dummies}
...
\midrule
AMBMP & 0.007 & 0.0006 & 11.071 & 0.000 & 1.001 \\
...
The regression results indicate that AMBMP maintains a significant positive relationship with future returns ($\beta = 0.007, p < 0.01$) even after controlling for market regimes. The bear market dummy is significantly negative ($\delta_{\text{bear}} = -0.003, p < 0.01$), suggesting that during bear markets, returns are generally lower. However, the interaction between market regimes and AMBMP (though not directly shown via interaction terms in this specification) implies that the effect of ambiguity might be modulated by market sentiment. These findings underscore the importance of accounting for macroeconomic conditions when assessing the role of ambiguity in asset pricing.

\subsubsection{Volatility Regimes: High vs. Low Realized Volatility} \label{subsec:volatility_regimes}

Volatility is a key characteristic of financial markets and often clusters during periods of market stress. We next examine whether the predictive power of AMBMP varies across high and low volatility environments. We split the sample into subgroups based on the 33rd and 66th percentiles of realized volatility (RV): the low-volatility subgroup (RV below the 33rd percentile) and the high-volatility subgroup (RV above the 66th percentile). The regression model is estimated separately for each subgroup:
...
AMBMP exhibits a significant positive relationship with future returns in both low and high plevel subgroups, with coefficients of 0.006 ($t = 5.662, p < 0.01$) and 0.007 ($t = 7.223, p < 0.01$), respectively. This consistency indicates that the return predictability of ambiguity is not simply an artifact of relative price levels. Yet the stronger coefficient among high-plevel stocks implies that ambiguity is incorporated into prices more swiftly for expensive, high-attention names. That pattern fits a momentum-driven "chase-and-kill" (or "buy-high, sell-low") trading style: when information is unclear, investors pile into the most visible, high-priced stocks, pushing prices even higher in the short run—a textbook case of "momentum chasing" behavior in China's A-share market.

\subsubsection{Summary of Heterogeneity Analysis}

The results in this subsection demonstrate that the predictive power of AMBMP for future returns is robust across various market environments, including different market regimes, volatility conditions, and price levels. The consistency of these findings strengthens the validity of AMBMP as a measure of ambiguity and highlights its potential utility in portfolio management and risk assessment under diverse market conditions.
...
\clearpage
\appendix
\section{Cross-Entropy Ambiguity Index: Concept and Computation}

\subsection*{Idea}

The Core Idea: Comparing Entire Distributions
Traditional risk measures, like realized volatility, skewness, and kurtosis, are moments of a distribution.

\begin{itemize}
    \item Realized Volatility measures the dispersion or spread of returns around the mean.
    \item Skewness measures the asymmetry of the distribution's shape.
    \item Kurtosis measures the "tailedness" or the presence of extreme outliers. 
\end{itemize}

While these metrics provide valuable information, they only offer a snapshot of a specific characteristic. They can fail to capture complex, simultaneous changes across the entire distribution.

In contrast, the CEA, based on Kullback-Leibler (KL) divergence, is a measure of the statistical "distance" between two entire probability distributions, rather than just a comparison of their moments. The formula for KL divergence is: 
...
In the context of your research, P(x) represents the observed, high-frequency intraday return distribution for a specific day, and Q(x) represents a benchmark or reference distribution derived from historical data. The final value of the measure is a single number that quantifies the total difference between the observed distribution for a single day and a historically plausible distribution. This is how the measure captures "full-shape changes." 

How it Captures Tail Behavior (Heavy Tails)

The Kullback–Leibler (KL) divergence is particularly sensitive to differences in the tails of two distributions. The formula takes the logarithm of the ratio of probabilities at every point $x$. This logarithmic ratio becomes extremely large when an outcome is observed in the actual distribution $P$ but is very rare or even impossible in the benchmark distribution $Q$.

Specifically, if a stock experiences an extreme, high-magnitude return (a ``tail event'') on a given day, the probability $P(x)$ for that return $x$ will be non-zero. If the benchmark distribution $Q(x)$ models that same event as highly improbable (i.e., $Q(x) \approx 0$), the term 
\[
\log \frac{P(x)}{Q(x)}
\]
will become very large, significantly increasing the total KL divergence.  

This means the CEA measure is highly responsive to the presence of \emph{fat tails}—where extreme events occur more frequently than a standard model would predict. It inherently penalizes a model that fails to anticipate these tail risks, making it a more robust measure for capturing sudden changes in market uncertainty and instability.  

How it Captures Skewness (Asymmetry)

The KL divergence measure is \emph{asymmetric}; that is,
\[
D_{\mathrm{KL}}(P \parallel Q) \neq D_{\mathrm{KL}}(Q \parallel P).
\]
This asymmetry is a key feature that allows it to distinguish between a long right-tail (positive skew) and a long left-tail (negative skew) in a way that symmetric measures cannot.  

When comparing an observed daily return distribution $P$ to a symmetric benchmark distribution $Q$ (e.g., a normal or $t$-distribution), the resulting KL divergence will implicitly capture any asymmetry in $P$. If $P$ has a long right tail, the positive returns will contribute more significantly to the divergence value than the negative ones, and vice versa. This ensures that the measure is sensitive not only to the magnitude of return deviations but also to their direction.

By leveraging high-frequency intraday data, the CEA measure constructs a detailed, fine-grained view of the entire distribution on a given day. The KL divergence then takes this full-shape distribution and compares it to a benchmark, providing a single, interpretable value that quantifies the ``information lost'' by assuming the market is behaving according to the benchmark model. This is how it captures the nuanced distributional changes that are vital for dynamic portfolio management.

\subsection*{Ambiguity Index Calculation}
The ambiguity index is computed through a two-stage process, with K-means clustering forming the core methodology for market state classification. The implementation uses a fixed 22-day sliding window to dynamically segment return distributions into four canonical market regimes.

\subsubsection*{K-means Clustering Implementation}
Daily return distributions (202-dimensional probability vectors from frequency binning over $[-20.1\%, 20.1\%]$) are clustered into $k=4$ market regimes using the K-means algorithm with Euclidean distance as the similarity metric. The clustering process minimizes the within-cluster sum of squared Euclidean distances:
...
where:
\begin{itemize}
\item $x$: Distribution vector for a given day
...
\begin{itemize}
\item \textbf{Window size}: 22 trading days (standard market cycle duration)
\item \textbf{Cluster count}: $k=4$ (empirically determined to capture distinct market regimes)
\item \textbf{Distance metric}: Euclidean distance between 202-dimensional distribution vectors
\item \textbf{Output}: Cluster centroids $C_{\text{window}} \in \mathbb{R}^{4 \times 202}$ representing market regimes
\end{itemize}

The clustering process is implemented as:
\begin{verbatim}
KMeans(n_clusters=4).fit(window_distributions)
\end{verbatim}

\subsubsection*{Ambiguity Quantification}
The ambiguity index $\text{ambiguity}_t$ is derived from the K-means output:

\[
\text{ambiguity}_t = \min_{c \in C_{\text{window}}} \text{relative entropy}(p_t, c)
\]
where lower values indicate higher similarity to historical market states.

K-means clustering provides the essential foundation for quantifying market uncertainty through dynamic regime classification, with Euclidean distance-based clustering ensuring robust segmentation of return distribution patterns.
\section{Concluding Summary of CEA}
