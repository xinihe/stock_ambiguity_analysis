...
\subsection{Visual Illustrations}

This appendix provides visual illustrations to enhance understanding of the concepts discussed in the paper.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\linewidth]{fig/figure1_kl_divergence.pdf}
    \caption{Kullback-Leibler Divergence Concept. The figure illustrates how KL divergence measures the difference between probability distributions under various scenarios including different means, variances, and distributional shapes.}
...
\begin{figure}[h!]
    ...
    \caption{Ambiguity Index Dynamics and Market Conditions. Panel (a) shows market index performance over time. Panel (b) displays the time-varying ambiguity index with periods of high and low uncertainty. Panel (c) illustrates the relationship between ambiguity levels and market returns.}
    \label{fig:ambiguity_dynamics}
\end{figure}

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\linewidth]{fig/figure3_portfolio_performance.pdf}
    \caption{Portfolio Performance Evaluation. The figure compares the performance of different strategies including the CSI 300 benchmark, AMBE strategy, and CEA strategy across multiple metrics including cumulative returns, drawdowns, and annual performance.}
    \label{fig:portfolio_performance}
\end{figure}

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\linewidth]{fig/figure4_risk_vs_ambiguity.pdf}
    \caption{Risk vs Ambiguity: Conceptual Distinction. Panel (a) illustrates traditional risk measures based on known distributions. Panel (b) shows ambiguity arising from multiple plausible distributions. Panel (c) demonstrates different correlation patterns with returns. Panel (d) summarizes key differences.}
    \label{fig:risk_vs_ambiguity}
\end{figure}

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.85\linewidth]{fig/figure5_methodology_flowchart.pdf}
    \caption{Cross-Entropy Ambiguity Index: Methodology Framework. The flowchart illustrates the step-by-step process of calculating the ambiguity index from intraday data collection to final application in portfolio optimization.}
...
In contrast, the ambiguity measure \(\mathcal{A}^{CEA}_t\), based on Kullback-Leibler (KL) divergence, is a measure of the statistical "distance" between two entire probability distributions, rather than just a comparison of their moments. The formula for KL divergence is: 
