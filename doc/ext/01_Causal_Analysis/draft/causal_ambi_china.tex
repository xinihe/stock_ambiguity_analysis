...
\noindent
\textbf{H1: A higher Cross-Entropy Ambiguity (CEA) leads to higher subsequent returns to compensate for the discomfort of unknown probabilities.}
...
\noindent
\textbf{H2: The ambiguity premium is significantly larger for brown energy stocks compared to green energy stocks.}
...
\noindent
\textbf{H3: Major policy clarifications will widen the ambiguity premium differential between brown and green stocks.}
...
\noindent
\textbf{H4: Ambiguity increases bid-ask spreads, which in turn predicts higher subsequent returns.}
...
\noindent
\textbf{H5: Geopolitical instability transmits to energy markets through an ambiguity channel.}
...
\begin{equation}
\begin{split}
\mathcal{A}^{CEA}_{i,t} = D_{KL}(q_{i,t} \| P_{w+1}) \\
= \sum_{j=1}^{202} q_{i,t}(x_j) \log\left(\frac{q_{i,t}(x_j)}{P_{w+1}(x_j)}\right).
\end{split}
\end{equation}
Numerical stability is maintained by adding a small constant $\epsilon = 10^{-10}$ to prevent division by zero:
\begin{equation}
\begin{split}
\mathcal{A}^{CEA}_{i,t} = \sum_{j: q_{i,t}(x_j) > 0} q_{i,t}(x_j) \\
\times \log\left(\frac{q_{i,t}(x_j)}{\max(P_{w+1}(x_j), \epsilon)}\right).
\end{split}
\end{equation}
...
\begin{align}
\intertext{Path a (Ambiguity $\to$ Liquidity):}
\text{Spread}_{i,t} &= a_0 + a_1 \mathcal{A}^{CEA}_{i,t} + \mathbf{c}'\mathbf{Controls}_{i,t} + \nu_{i,t}, \\
\intertext{Path b (Liquidity $\to$ Returns):}
\begin{split}
