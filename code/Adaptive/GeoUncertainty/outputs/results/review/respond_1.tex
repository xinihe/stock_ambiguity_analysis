...
\begin{enumerate}
    \item \textbf{The causal interpretation in the mediation analysis relies on strong assumptions and may be under-identified given potential confounders and timing frictions; clarify identification assumptions and timing choices, and provide a brief narrative explaining why the observed patterns are consistent with the intended causal pathway.}

    \begin{responsebox}
    \vspace{0.5cm}
    \textbf{Response:} We acknowledge the reviewer's important concern about causal identification in our mediation analysis. We have revised the manuscript to explicitly address these issues:
    \vspace{0.5cm}

    \textbf{1. Identification Assumptions:}
    We have clearly stated that our mediation analysis relies on three key assumptions:
    \begin{itemize}
    ...
    \begin{responsebox}
    \vspace{0.5cm}
    \textbf{Response:} We thank the reviewer for highlighting this important methodological point. We have made the following additions:
    \vspace{0.5cm}

    \textbf{1. Benchmark Selection Rationale:} We have clarified that:  
    \begin{itemize}
    ...
    \begin{responsebox}
    \vspace{0.5cm}
    \textbf{Response:} We have added a dedicated ``Data Timing and Alignment'' subsection with explicit details:
    ...
    We have added descriptive evidence:
    ...
    We have tested and ruled out size effects by adding:
    ...
    \begin{itemize}
        \item \textbf{Economic Significance:} We now explicitly state: ``A one-standard-deviation increase in ambiguity reduces returns significantly, underscoring its economic relevance.''
        \item \textbf{Comparative Magnitude:} We highlight that ``including ambiguity alongside volatility improves model fit and reduces the volatility-only coefficient,'' underscoring why the ambiguity channel is non-negligible.
    \end{itemize}
...
\begin{enumerate}
    ...
    \begin{responsebox}
    \vspace{0.5cm}
    \textbf{Response:} We have added precise dates and expanded the description to ensure transparency and reproducibility:
    ...
    \item \textbf{Define all variables in table captions (e.g., ``$\Delta$Ambiguity'', ``RV'') and specify units/frequencies.}

    \begin{responsebox}
    \vspace{0.5cm}
    \textbf{Response:} We have updated all table captions with complete definitions and units/frequencies, and described transformation rules used in the analysis:
    ...
    \begin{responsebox}
    \vspace{0.5cm}
    \textbf{Response:} We have implemented uniform readability and caption standards:
    \begin{itemize}
        \item Increased table font sizes from 9pt to 10pt and adjusted column spacing for improved readability at journal column widths
        \item Ensured all captions include: ``Standard errors are Newey–West HAC robust with 5-day lag (unless otherwise specified)''
        \item Added: ``*, **, and *** denote significance at the 10\%, 5\%, and 1\% levels, respectively''
        \item Verified axis label sizes and tick formatting for figures to maintain consistency with table readability standards
    \end{itemize}
    ...
    \begin{responsebox}
    \vspace{0.5cm}
    \textbf{Response:} We have standardized notation and cross-checked usage throughout:
    \begin{itemize}
        ...
        \item Updated all tables and equations to use this notation and confirmed consistency in the mediation and robustness sections
    \end{itemize}
    ...
    \begin{responsebox}
    \vspace{0.5cm}
    \textbf{Response:} We have added Algorithm 1 to Appendix A with inputs/outputs and procedural clarity:
    ...
    \begin{responsebox}
    \vspace{0.5cm}
    \textbf{Response:} We have added to the IV section and briefly summarized diagnostics:

    \textit{``We report standard IV diagnostics indicating strong instruments and validity: first-stage statistics exceed conventional thresholds, and overidentification tests fail to reject instrument exogeneity.''}
    ...
    \begin{responsebox}
    \vspace{0.5cm}
    \textbf{Response:} We have added recent references and integrated them into the comparative discussion:
