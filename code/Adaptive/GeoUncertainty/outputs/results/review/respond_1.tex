...
We sincerely thank the reviewer for their insightful comments and constructive suggestions. We have carefully addressed each point and revised the manuscript accordingly. Below we provide detailed point-by-point responses.
...
\begin{enumerate}
    ...
    \textbf{4. Additional Robustness:}
    We will add instrumental variable results using non-Asian GPR as an exogenous shock to further support causal claims.
    }}
    ...
    \colorbox{responsebg}{
    \parbox{0.95\textwidth}{
    \textbf{Response:} We will strengthen the SOE analysis as follows:

    \textbf{1. SOE Definition:}
    We will add:

    \textit{``SOEs are classified based on the China Securities Regulatory Commission (CSRC) definition: firms where the state is the ultimate controlling shareholder (>50\% direct ownership or >30\% through pyramid structures). We verify this using the WIND database ownership classifications and cross-check with government stake announcements.''}

    \textbf{2. Sample Characteristics:}
    We will add Table X summarizing:
    \begin{itemize}
    ...
    \colorbox{responsebg}{
    \parbox{0.95\textwidth}{
    \textbf{Response:} We will revise to highlight our unique contributions:
    ...
    We will focus on incremental insights:

    \begin{itemize}
        \item Remove redundant statistical details
        \item Emphasize economic significance: ``A one-standard-deviation increase in ambiguity reduces returns by 11.3 bps daily---equivalent to 28.3\% annualized''
        \item Compare with volatility: ``Ambiguity's impact is 1.4x larger than volatility's, despite having lower variance''
    \end{itemize}
...
\begin{enumerate}
    ...
    \colorbox{responsebg}{
    \parbox{0.95\textwidth}{
    \textbf{Response:} We will add precise dates:
    ...
    \colorbox{responsebg}{
    \parbox{0.95\textwidth}{
    \textbf{Response:} We will update all table captions with complete definitions:
    ...
    \colorbox{responsebg}{
    \parbox{0.95\textwidth}{
    \textbf{Response:} We will:
    \begin{itemize}
        \item Increase all table fonts from 9pt to 10pt
        \item Ensure all captions include: ``Standard errors are Newey-West HAC robust with 5-day lag (unless otherwise specified)''
        \item Add note: ``*, **, and *** denote significance at the 10\%, 5\%, and 1\% levels, respectively''
    \end{itemize}
    }}
    ...
    \colorbox{responsebg}{
    \parbox{0.95\textwidth}{
    \textbf{Response:} We will standardize notation:
    \begin{itemize}
        ...
        \item Update all tables and equations to use this notation
    \end{itemize}
    }}
    ...
    \colorbox{responsebg}{
    \parbox{0.95\textwidth}{
    \textbf{Response:} We will add Algorithm 1 to Appendix A:
    ...
    \begin{itemize}
        \item \citet{Davis2023} on geopolitical uncertainty spillovers
        \item \citet{Liu2024} comparing EPU, GPR, and ambiguity
        \item \citet{Zhang2024} on high-frequency uncertainty measures
        \item \citet{Wang2023} on machine learning approaches to uncertainty
    \end{itemize}
