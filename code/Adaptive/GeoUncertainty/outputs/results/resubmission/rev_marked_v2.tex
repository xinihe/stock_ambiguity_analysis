...
\textcolor{black}{While existing literature predominantly examines GPR through the lens of volatility \citep{Salisu2021}, we demonstrate that ambiguity represents a distinct and economically significant transmission channel. Unlike volatility---which captures second-moment risk with known probabilities---ambiguity reflects Knightian uncertainty where probability distributions themselves are unknown. Our framework shows that: \textcolor{red}{(i) GPR significantly increases both ambiguity and volatility; (ii) ambiguity operates alongside volatility to explain GPR's effect on returns; and (iii) models ignoring ambiguity misattribute a portion of ambiguity's impact to volatility, illustrating the complementary nature of these two uncertainty channels.}}
...
Our research is situated at the intersection of two burgeoning fields: the asset pricing implications of ambiguity and the impact of geopolitical risk (GPR) on financial markets. We build a bridge between them by proposing that \textcolor{red}{ambiguity is a critical, yet under-appreciated, channel} through which geopolitical tensions transmit to equity returns.
...
\textcolor{red}{Our work extends the emerging literature on uncertainty transmission in several critical ways, bridging the gap between macroeconomic uncertainty and ambiguity pricing. First, while studies like \citet{Baker2016} and \citet{Caldara2022} document that uncertainty matters for markets, they often treat uncertainty as monolithic. By decomposing uncertainty into complementary risk (volatility) and ambiguity channels, we build on the insights of \citet{Brenner2018} and reveal that ambiguity is a distinct factor in GPR's pricing effect.} \textcolor{black}{Second, unlike traditional proxy-based ambiguity measures \citep{Brenner2018}, our cross-entropy-based approach captures real-time model uncertainty at daily frequency. Third, by examining the SOE heterogeneity, we uncover institutional mechanisms that mitigate ambiguity pricing---a dimension absent from volatility-focused studies.}
...
\section*{Appendix B: Comparison with Alternative Ambiguity Proxies}
...
\textcolor{red}{\textbf{Forecast Dispersion.} A prominent approach uses the dispersion of professional macroeconomic or earnings forecasts to proxy for ambiguity \citep{Ulrich2013, Brenner2018}. The intuition is that when experts disagree, the true data-generating process is highly uncertain, reflecting Knightian ambiguity. For example, \citet{Bachmann2013} utilize survey data to construct measures of business uncertainty. However, these survey-based measures face several limitations. First, they are constrained by analyst coverage, which heavily biases samples toward large-cap stocks and makes it difficult to construct comprehensive cross-sectional measures, especially in emerging markets where coverage is sparse. Second, forecast dispersion is typically only available at lower frequencies (monthly or quarterly), limiting its usefulness for capturing rapid shifts in ambiguity during geopolitical crises. In contrast, our cross-entropy measure relies solely on high-frequency market prices, allowing for daily construction across the entire universe of liquid stocks without subjective biases.}

\textcolor{red}{\textbf{Variance-of-Variance (VoV) and Macroeconomic Uncertainty.} Another common proxy is the variance of variance or the volatility of implied volatility (e.g., the VVIX index). While related to uncertainty, VoV fundamentally captures the second moment of the volatility process—it is a measure of risk about risk, rather than ambiguity about the probability distribution itself. Similarly, macroeconomic uncertainty measures, such as the widely used index by \citet{Jurado2015}, extract the unforecastable component of a large set of macroeconomic indicators. While powerful for business cycle analysis, these measures often conflate risk (forecast error variance) with true Knightian uncertainty.}

\textcolor{red}{\textbf{The Cross-Entropy Contribution.} Our cross-entropy-based measure, $\mathcal{A}_t$, offers a distinct methodological contribution by directly operationalizing model uncertainty. Rooted in information theory, the Kullback-Leibler divergence does not merely measure the spread of outcomes (risk) or the disagreement among a subset of agents (dispersion). Instead, it quantifies the degree to which the currently observed market distribution deviates from all plausible historical reference models. When $\mathcal{A}_t$ is high, it indicates that current market dynamics cannot be reliably described by past paradigms—the essence of Knightian uncertainty. By computing this divergence using intraday data, we provide a mathematically rigorous, objective, and high-frequency metric that captures a dimension of uncertainty fundamentally distinct from both subjective disagreement and higher-order risk. Crucially, by isolating this specific dimension, our approach explicitly bridges the macroeconomic uncertainty literature (which typically relies on textual or volatility-based proxies) with the theoretical asset-pricing literature on ambiguity aversion.}
