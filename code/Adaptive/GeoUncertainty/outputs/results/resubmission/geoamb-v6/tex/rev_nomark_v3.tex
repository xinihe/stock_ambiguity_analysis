...
\textcolor{black}{While existing literature predominantly examines GPR through the lens of market volatility \citep{Liu2024} and tail risk \citep{Salisu2021}}, we demonstrate that ambiguity represents a distinct and economically significant transmission channel. Unlike volatility---which captures second-moment risk with known probabilities---ambiguity reflects Knightian uncertainty where probability distributions themselves are unknown. Our framework shows that: \textcolor{black}{(i) GPR significantly increases both ambiguity and volatility; (ii) ambiguity operates alongside volatility to explain GPR's effect on returns; and (iii) models ignoring ambiguity misattribute a portion of ambiguity's impact to volatility, illustrating the complementary nature of these two uncertainty channels.}

Our goal is to provide a comprehensive account of how GPR affects returns through both channels. We construct our daily ambiguity measures using a novel, simplified cross-entropy-based approach, which we argue is more robust than traditional proxies like the variance of variance. \textcolor{black}{By doing so, we bridge the macroeconomic uncertainty literature---which often treats uncertainty as monolithic---with the asset-pricing literature on Knightian ambiguity. A detailed comparison of our measure against alternative proxies is provided in Appendix B.} This methodological contribution, detailed in Appendix A, is the foundation for our empirical tests.
...
Our research is situated at the intersection of two burgeoning fields: the asset pricing implications of ambiguity and the impact of geopolitical risk (GPR) on financial markets. \textcolor{black}{ We build a bridge between them by proposing that ambiguity is a critical, yet under-appreciated, channel through which geopolitical tensions transmit to equity returns.}
...
\textcolor{black}{Our work extends the emerging literature on uncertainty transmission in several critical ways, bridging the gap between macroeconomic uncertainty and ambiguity pricing. First, while studies like \citet{Baker2016} and \citet{Caldara2022} document that uncertainty matters for markets, they often treat uncertainty as monolithic. By decomposing uncertainty into complementary risk (volatility) and ambiguity channels, we build on the insights of \citet{Brenner2018} and reveal that ambiguity is a distinct factor in GPR's pricing effect.} \textcolor{black}{Second, unlike traditional proxy-based ambiguity measures \citep{Brenner2018}, our cross-entropy-based approach captures real-time model uncertainty at a daily frequency. Third, by examining the SOE heterogeneity, we uncover institutional mechanisms that mitigate ambiguity pricing---a dimension absent from volatility-focused studies.}
...
This hypothesis introduces a key element of heterogeneity. We conjecture that the implicit government guarantees and policy roles of SOEs may buffer them from the full impact of ambiguity. \textcolor{black}{This buffering effect should result in a less pronounced negative relationship between ambiguity and returns for these firms compared to their non-SOE counterparts.}
...
\textcolor{black}{We use a contemporaneous alignment $(t, t)$ for our main analysis. This is justified because the GPR news (compiled at 00:00 UTC, arriving between 5:00 AM and 8:00 AM Beijing time) provides sufficient time for this information to be incorporated into investor beliefs and trading behavior throughout the trading day before the 15:00 close. However, recognizing that information frictions or behavioral delays might cause market reactions to spill over, we also introduce a lagged specification $(t-1, t)$ in our robustness checks. This verifies that the negative pricing of ambiguity is robust and not merely an artifact of our specific contemporaneous timing assumptions.}

For the cross-sectional analysis, we rely on standard firm-level characteristics known to explain the cross-section of stock returns. We control for firm size, measured as the natural logarithm of market capitalization, and the book-to-market ratio to account for the size and value premiums, respectively. To capture return persistence, we include momentum, defined as the cumulative return over the past 12 months excluding the most recent month. \textcolor{black}{Finally, given the unique institutional context of China's economy, we define an SOE dummy (set to 1 for state-owned enterprises), which we later introduce in our moderation analysis to explore the influence of ownership structure.}
...
\begin{tablenotes}[flushleft]
\small
\item[] This table shows the impact of GPR shocks on financial ambiguity. The dependent variable is the daily change in our cross-entropy-based ambiguity measure. Control variables include lagged ambiguity, market returns, the VIX, and term spread. Standard errors are Newey-West HAC robust with 5-day lag.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
where $R_t$ is the CSI 300 Index return on day $t$, $\Delta \text{GPR}_t$ is the geopolitical risk shock, $\Delta \mathcal{A}_t$ is the ambiguity shock, $\Delta \text{RV}_t$ is the change in realized volatility, and \textcolor{black}{$\mathbf{X}_t$ includes market return, the VIX, term spread, and risk-free rate.}

We define the shock operator $\Delta V_t$ as the first difference $\Delta V_t = V_t - V_{t-1}$; thus $\Delta \mathcal{A}_t$ and $\Delta \text{RV}_t$ represent the daily changes in ambiguity and realized volatility.
...
\textcolor{black}{where $\mathbf{Z}_t$ includes control variables such as lagged ambiguity, market return, the VIX, and term spread.}

\paragraph{\textcolor{black}{Empirical Results}}
\textcolor{black}{Table \ref{tab:baseline_ts} reports the results. We observe a negative and significant relationship between ambiguity and returns.} \textcolor{black}{In terms of economic significance, a one-standard-deviation increase in ambiguity is associated with a daily return reduction of 11.3 bps. We focus on this daily impact, which is more appropriate than annualized figures given the high-frequency nature of our data and the transient nature of the shocks we study.} \textcolor{black}{As shown in Table \ref{tab:gpr_ambiguity}, GPR significantly increases financial ambiguity across the full sample. Notably, this effect remains robust across both the Pre-COVID and COVID sub-periods, a point we will further elaborate on in our robustness checks.}
...
\textcolor{black}{In the second stage, consistent with the empirical design in \citet{Fama1992Cross}, we use these estimated betas as independent variables to run cross-sectional regressions for each period. We incorporate standard firm-level characteristics as control variables:}
...
\textcolor{black}{where $R_{i,t}$ is the return on stock $i$ at time $t$. The term $\mathbf{F}_{i,t}$ represents the firm characteristics included as controls, specifically firm size, book-to-market ratio, and momentum. Table \ref{tab:fama_macbeth} reports the Fama-MacBeth cross-sectional risk premiums. Consistent with our hypothesis (H2), the ambiguity beta carries a negative and highly significant risk premium across individual stocks, confirming that ambiguity is a systematically priced factor.}
...
FS & -0.419*** & -4.46 & -0.452*** & -4.92 & -0.387*** & -4.21 \\
...
\textcolor{black}{\paragraph{Alternative Specifications and Summary of Effects} As detailed in Table \ref{tab:robustness}, we further verify our findings against a comprehensive set of alternative specifications. To ensure our results are not artifacts of measurement choices, we test alternative ambiguity measures constructed with different historical benchmark model weights, and replace our baseline high-frequency realized volatility with alternative volatility measures. We also verify stability using alternative market beta estimations. A comprehensive summary of the main effects across all key model specifications is provided in Appendix C (Table \ref{tab:summary_effects})}

\section{Conclusion}
\label{sec:conclusion}

This paper analyzes how global Geopolitical Risk (GPR) impacts Chinese equity markets, uniquely differentiating between volatility (traditional risk) and ambiguity (Knightian uncertainty)—filling the gap in existing literature that treats such uncertainty as a monolith. Using a new daily ambiguity measure based on simplified cross-entropy, we identify three key insights: \textcolor{black}{ambiguity is a distinct priced factor in China's stock market (driving CSI 300 declines independently of volatility and acting as a critical transmission channel for GPR)}; state-owned enterprises (SOEs) are less affected by ambiguity (supporting the "government buffer" hypothesis, while private firms bear full risks); and "Non-Asian" overnight GPR shocks (used as an instrumental variable) confirm our causal results.

\textcolor{black}{These findings offer clear implications: investors may benefit from ambiguity-aware risk management (as volatility-only models could leave portfolios exposed during geopolitical shocks), while policymakers should use ambiguity-reducing communication and targeted non-SOE support to mitigate GPR's impacts.} Future research could extend the cross-entropy-based ambiguity measure to bond/currency markets or explore its role across geopolitical regimes (e.g., trade wars vs. military conflicts).
...
\section{\textcolor{black}{Comparison with Alternative Ambiguity Proxies}}
\label{app:alternative_proxies}
\renewcommand{\theequation}{B.\arabic{equation}}
\setcounter{equation}{0}

\textcolor{black}{While Section \ref{sec:literature} briefly contrasts our cross-entropy measure with existing ambiguity proxies, this appendix provides a more comprehensive methodological and empirical comparison to highlight the explicit contribution of our approach.}

\textcolor{black}{The empirical asset pricing literature has traditionally relied on two main classes of ambiguity proxies: those based on the dispersion of subjective beliefs, and those based on the volatility of volatility.}

\textcolor{black}{\textbf{Subjective Belief Dispersion.} Existing literature often relies on the dispersion of professional macroeconomic or earnings forecasts to proxy for Knightian ambiguity \citep{Ulrich2013}, operating on the premise that expert disagreement reflects an underlying uncertainty in the true data-generating process. For instance, survey data has been utilized to construct measures of business uncertainty \citep{Bachmann2013}, while other empirical studies measure the volatility of return probabilities to demonstrate ambiguity pricing in equities \citep{Brenner2018}. However, these survey-based measures face significant structural limitations. First, they are constrained by analyst coverage, which heavily biases samples toward large-cap stocks and restricts the construction of comprehensive cross-sectional measures—a problem particularly acute in emerging markets. Second, forecast dispersion is typically only available at lower frequencies (monthly or quarterly), rendering it inadequate for capturing the rapid, daily shifts in ambiguity that characterize geopolitical crises.}

\textcolor{black}{\textbf{Volatility-Based and Macroeconomic Proxies.} A second prominent strand of literature employs statistical and macro-level proxies, such as the variance-of-variance (e.g., the VVIX index) or broad macroeconomic uncertainty indices \citep{Jurado2015}. While these metrics are powerful tools for business cycle analysis, they conceptually conflate standard risk with true Knightian uncertainty. Variance-of-variance fundamentally captures the second moment of the volatility process; it is a measure of the "risk about risk" rather than ambiguity regarding the probability distribution itself. Similarly, macroeconomic measures that extract the unforecastable components of large indicator sets tend to capture forecast error variance (risk) rather than situations where the probabilities remain entirely unknown.}

\textcolor{black}{\textbf{The Cross-Entropy Contribution.} To overcome the limitations of subjective biases and higher-order risk proxies, our cross-entropy-based measure, $\mathcal{A}_t$, offers a distinct methodological innovation by directly operationalizing model uncertainty. Rooted in information theory, the Kullback-Leibler divergence does not merely measure the spread of outcomes (risk) or the disagreement among a subset of agents (dispersion). Instead, it quantifies the exact degree to which the currently observed market distribution deviates from all plausible historical reference models. When $\mathcal{A}_t$ is high, it objectively indicates that current market dynamics cannot be reliably described by past paradigms—the very essence of Knightian uncertainty. By computing this divergence using high-frequency intraday market prices, we provide a mathematically rigorous and daily metric that broadly spans the entire universe of liquid stocks without subjective bias. Crucially, by isolating this specific dimension of uncertainty, our approach explicitly bridges the macroeconomic uncertainty literature (which typically relies on textual or volatility-based proxies) with the theoretical asset-pricing literature on ambiguity aversion.}

\section{\textcolor{black}{Comprehensive Summary of Robustness Checks}}
\label{app:summary_effects}
...
\textcolor{black}{Table \ref{tab:summary_effects} provides a simplified summary of the core coefficients across all key model specifications. Across all tested models—including the Pre-COVID and COVID sub-periods, as well as the instrumental variable approach—the negative impact of ambiguity remains consistent and highly significant, confirming its robust role as a pricing factor.}

\begin{table}[htbp]
\centering
\color{black}
\captionsetup{labelfont={color=black}}
\caption{\textcolor{black}{Summary of Main Effects Across Model Specifications}}
