...
\textcolor{red}{While existing literature predominantly examines GPR through the lens of volatility \citep{Bouri2021, Apergis2021}, we demonstrate that ambiguity represents a distinct and economically significant transmission channel. Unlike volatility---which captures second-moment risk with known probabilities---ambiguity reflects Knightian uncertainty where probability distributions themselves are unknown. Our framework shows that: (i) GPR increases ambiguity as a separate channel alongside volatility; (ii) ambiguity explains 38.5\% of GPR's effect on returns; and (iii) models ignoring ambiguity misattribute ambiguity's impact to volatility.}
...
\small
\item[] This table presents the baseline time-series regression results of CSI 300 Index returns on geopolitical risk (GPR) shocks, ambiguity shocks, and volatility shocks. Standard errors are Newey-West HAC robust with 5-day lag. *, **, and *** denote significance at the 10\%, 5\%, and 1\% levels, respectively.\textcolor{red}{The 5-day lag in Newey-West covariance estimation accounts for serial correlation up to one trading week, improving inference in daily data where residuals often exhibit short-run autocorrelation. We confirmed this choice using residual diagnostics and robustness checks; results are qualitatively similar for alternative lag selections.}
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\textcolor{red}{Table \ref{tab:baseline_ts} reports the results. We observe a negative and significant relationship between ambiguity and returns. Including ambiguity alongside volatility improves model fit and reduces the volatility-only coefficient, underscoring why the ambiguity channel is non-negligible.}
...
\begin{equation}
R_{i,t} = \alpha_{i,t} + \lambda_{1,t} \beta_{i,\text{MKT}} + \lambda_{2,t} \textcolor{red}{\beta_{i,\text{AMB}}} + \lambda_{3,t} \textcolor{red}{\beta_{i,\text{VOL}}} + \Gamma_{t}' \mathbf{F}_{i,t} + \epsilon_{i,t}
\end{equation}

where $R_{i,t}$ is the return on stock $i$ at time $t$, $\beta_{i,\text{MKT}}$ is the market beta, \textcolor{red}{$\beta_{i,\text{AMB}}$} is the ambiguity beta (sensitivity to ambiguity shocks), \textcolor{red}{$\beta_{i,\text{VOL}}$} is the volatility beta, and $\mathbf{F}_{i,t}$ includes firm characteristics.

The ambiguity beta is estimated from a first-pass time-series regression:
\begin{equation}
R_{i,t} = \alpha_i + \beta_{i,\text{MKT}} \text{MKT}_t + \textcolor{red}{\beta_{i,\text{AMB}}} \Delta \mathcal{A}_t + \textcolor{red}{\beta_{i,\text{VOL}}} \Delta \text{RV}_t + \epsilon_{i,t}
\end{equation}
...
\textcolor{red}{We report standard IV diagnostics indicating strong instruments and validity: first-stage statistics exceed conventional thresholds, and overidentification tests fail to reject instrument exogeneity. These diagnostics support the robustness of the IV results.}
...
{\color{red}
\paragraph{\textbf{Algorithm (Daily Ambiguity Calculation)}}

\textbf{Inputs:} Intraday price vector $P_{t}^{(1:m)}$; benchmark window $\mathcal{W}_{t-252:t-1}$\\

\textbf{Output:} Ambiguity measure $\mathcal{A}_t$\\

\textbf{Procedure:}
\begin{enumerate}
    \item For each day $d \in \mathcal{W}_{t-252:t-1}$, compute intraday returns $r^{(i)}_d = \log(P^{(i)}_d/P^{(i-1)}_d)$ and form an empirical distribution $\pi_d$.
    \item Compute the current distribution $\mu_t$ from day $t$ intraday returns.
    \item Calculate divergences $D_j = \mathcal{D}_{KL}(\mu_t \| \pi_j)$ for all benchmark distributions $\pi_j$.
    \item Return $\mathcal{A}_t = \min_j D_j$.
\end{enumerate}
\textbf{Complexity:} $O(m \times k)$, where $m$ is the number of intraday intervals and $k$ is the number of benchmark days.
}
\end{document}
